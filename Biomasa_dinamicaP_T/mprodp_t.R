...
inputs=read.csv("merluza2.csv",sep=",")
attach(inputs)
...
parini=log(c(K0,r0,sigma))

datos=data.frame(Año,Y,CPUEdat,p) # datos 
...
plot(CPUEpred[id],CPUEdat[id],type="p",main=paste("r=",round(cor(log(CPUEpred[id]),log(CPUEdat[id])),2)),

...
source('read.admbFit.R')
 ...
 Bio=matrix(NA,5,26)
 Rec=matrix(NA,5,26)
 Fmort=matrix(NA,5,26)
 deple=matrix(NA,5,26)
...
for (i in 1:5)
 {
  ...
  Bio[i,1:length(Biomasa_desovante[1,])]=Biomasa_desovante[1,]
  Rec[i,1:length(Biomasa_desovante[1,])]=Reclutamientos[2,]
  Fmort[i,1:length(Biomasa_desovante[1,])]=Mort_F[1,]
  deple[i,1:length(Biomasa_desovante[1,])]=SPR[1,]
